...
import {
  ...
  AspectRatioProps,
  AspectRatioWrapper,
} from '@/library/aspect-ratio/types';
 ...
 * @param {int} y
 */
const calculateAspectRatio = (x: number, y: number): string =>
  `${(y / x) * 100}%`;

const StyledAspectRatioWrapper = styled.div`
  display: block;
  max-width: ${(props: AspectRatioWrapper) =>
...
const StyledAspectRatioComponent = styled.div`
  display: block;
  width: 100%;
  padding-bottom: ${(props: AspectRatioComponent) =>
    calculateAspectRatio(props.x, props.y)};
  position: relative;
`;
 ...
 */
const injectStylesInChildComponent = (
  children: React.ReactElement
): React.ReactElement => {
  //Merge the children styles with the injected children styles
  const mergedChildrenStyles = {
    ...injectedChildrenStyles,
