...
const Wrapper = styled('nav')<HeaderStyle>`
  background: ${props => props.visible ? props.theme.navigation.backgroundMobile : "transparent"};
                    ...
                    <div className={headerStyles.menuGrid}>
                        {navigationPaths.map(({path, name}, i) => {
                            ...
                            if (i === navigationPaths.length - 1)
                                darkButton = (<DarkMode visible={true} darkmode={darkmode} toggleDarkmode={toggle}/>)

                            return (
                                <div className={headerStyles.gridCell}>
                                    <Link href={path}>
                                        <a onClick={() => setOpen(false)} className="hover-link">{name}</a>
                                    </Link>
                                    {darkButton}
                                </div>
                            )
